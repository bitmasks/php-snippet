...
function onDeleteAllJobsClick(event) {
    $.ajax({url: "./crontab.php?op=deleteall"}).done(function (data) {
        location.reload();
    });
}

function onDeleteJobClick(jobID) {
    $.post("./crontab.php?op=deletejob", { "jobid": jobID },
        function (data) {
            location.reload();
        }
    );
...
function onSaveButtonClick(event) {
    ...
    $.post("./crontab.php?op=add", { "minute": minute, "hour": hour, "dayweek": dayweek, "daymonth": daymonth, "month": month, "name": name, "command": command },
        function (data) {
            console.log(data);
            changePage('active');
        }
...
function getActiveCronjobs() {
    $.getJSON('./crontab.php?op=active', function (data) {
        var items = '';
